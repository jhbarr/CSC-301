...
bool testExchange(int exchangeNum, bool cycleExpected, bool verbosity)
{
    ...
    else
    {
        ...
        else
        {
            ...
            // Also check if there were any excess repeated verices (not start).
            double arb = 1;
            int c = 0;
            int r = 0;
            vector<int> repeated;
            for (int i = 0; i < cycle.size() - 1; i++)
            {
                ...
                c = count(cycle.begin(), cycle.end() - 1, cycle[i]);
                if (c > 1)
                {
                    passed = false;
                    r = count(repeated.begin(), repeated.end(), cycle[i]);
                    if (r == 0)
                    {
                        repeated.push_back(cycle[i]);
                    }
                }
            }
            if (repeated.size() > 0)
            {
                if (verbosity)
                {
                    cout << "\tRepeated currencies found in cycle:" << endl;
                    cout << "\t\t";
                    for (int i = 0; i < repeated.size() - 1; i++)
                    {
                        cout << currencies[repeated[i]] << ", ";
                    }
                    cout << currencies[repeated[repeated.size() - 1]] << endl;
                }
            }
            if (arb <= 1)
            {
...
int main(int argc, char *argv[])
{
    ...
    // pair ex = getRates(1);
    // vector<double> mat = ex.first;
    // vector<string> labels = ex.second;
    // cout << printCurrencyMatrix(mat, labels) << endl;
